...
if (require.main === module) {
  ...
  if (args.length < 6) {
    console.error("Usage: register_question.ts <question_text> <expected_expiration_time> <latest_expiration_time> <category> <rules> <early_resolution_threshold> [wallet_path]");
    ...
    console.error("  early_resolution_threshold: Floating point number (f64) for early resolution threshold");
    ...
    console.error("\nExample:");
    console.error("  register_question.ts 'Will Bitcoin reach $100k?' 1704063000 1704063500 'Crypto' 'Rule description here' 0.75");
    process.exit(1);
  }
  ...
  const rules = args[4];
  const earlyResolutionThreshold = parseFloat(args[5]);
  const walletPath = expandPath(
    args[6] || process.env.ANCHOR_WALLET || "~/.config/solana/id.json"
  );
